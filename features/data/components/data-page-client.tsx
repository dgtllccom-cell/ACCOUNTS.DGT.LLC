    ...
    <div className="space-y-4 p-4 lg:p-6" dir={isRtl ? "rtl" : "ltr"}>
      ...
      <Card>
        ...
        <CardContent className="grid gap-3 md:grid-cols-2 xl:grid-cols-6">
          ...
          <label className="space-y-1 text-xs font-semibold text-muted-foreground xl:col-span-2">{tt("common.search", "Search")}<div className="relative"><Search className="absolute left-3 top-2.5 h-4 w-4 text-muted-foreground" /><Input className="pl-9" value={q} onChange={(e) => setQ(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") loadData(); }} placeholder={tt("data.search_ph", "Account no, name, manual code, PO, SO, serial...")} /></div></label>
        </CardContent>
      </Card>
