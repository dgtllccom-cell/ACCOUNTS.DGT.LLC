...
export function DocumentManagerModal({
  ...
  const tt = (k: string, f: string) => t(lang, ("docmgr." + k) as never, f);
  ...
  const handleUpload = async (file: File, documentIdToReplace?: string) => {
    if (file.size > 20 * 1024 * 1024) {
      alert(tt("size_limit", "File size exceeds 20MB limit."));
      return;
    }
    ...
    try {
      ...
      alert(err.message || tt("upload_failed", "Failed to upload document"));
    } finally {
      setUploading(false);
  ...
  const handleDelete = async (docId: string) => {
    if (!confirm(tt("confirm_delete", "Are you sure you want to delete this document? This cannot be undone."))) return;
    try {
      setLoading(true);
      ...
      alert(tt("delete_failed", "Failed to delete document. You might not have permission."));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDownload = async (docId: string, versionId?: string) => {
    try {
      ...
      alert(tt("download_failed", "Failed to download document"));
    }
  };
      ...
      <DialogContent className="max-w-3xl p-0 overflow-hidden bg-slate-50 dark:bg-slate-950 flex flex-col h-[85vh] sm:h-[600px]">
        <DialogHeader className="px-6 py-4 border-b bg-white dark:bg-slate-900 shrink-0">
          <DialogTitle className="flex items-center gap-2 text-lg font-bold">
            ...
            {tt("title", "Document Manager")}
          </DialogTitle>
          <p className="text-xs text-muted-foreground mt-1">
            {tt("subtitle", "Manage attachments, versions, and uploads for this record.")}
          </p>
        </DialogHeader>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="rounded-xl border-2 border-dashed border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/50 p-8 flex flex-col items-center justify-center text-center transition-colors hover:border-primary/50 hover:bg-slate-50 dark:hover:bg-slate-800/50">
            ...
            <h3 className="text-sm font-semibold mb-1">{tt("upload_new_document", "Upload New Document")}</h3>
            <p className="text-xs text-muted-foreground mb-4 max-w-sm">
              {tt("upload_hint", "Drag and drop your files here or click to browse. Supported formats: PDF, Word, Excel, Images, ZIP. (Max 20MB)")}
            </p>
            <input
              type="file"
              ref={fileInputRef}
          ...
          <div className="space-y-3">
            <h3 className="text-sm font-bold flex items-center justify-between">
              {tt("attached_documents", "Attached Documents")} ({documents.length})
            ...
            {documents.length === 0 && !loading && (
              <div className="text-center py-10 bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800">
                ...
                <p className="text-sm font-medium text-slate-500">{tt("no_documents", "No documents attached yet")}</p>
              </div>
            )}
              ...
              <div key={doc.id} className="rounded-xl border bg-white dark:bg-slate-900 overflow-hidden shadow-sm">
                <div className="flex items-center gap-4 p-4">
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    ...
                    <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-500 hover:text-blue-600" title={tt("upload_new_version", "Upload New Version")} onClick={() => {
                    ...
                    {doc.versionCount > 1 && (
                      <Button variant="ghost" size="icon" className={cn("h-8 w-8 text-slate-500", expandedDocId === doc.id && "bg-slate-100 dark:bg-slate-800")} title={tt("version_history", "Version History")} onClick={() => setExpandedDocId(expandedDocId === doc.id ? null : doc.id)}>
                        <History className="h-4 w-4" />
                      </Button>
                    )}
                    <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-500 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-950/50" title={tt("delete_document", "Delete Document")} onClick={() => handleDelete(doc.id)}>
                ...
                {expandedDocId === doc.id && doc.versions && (
                  <div className="border-t bg-slate-50/50 dark:bg-slate-950/50 p-4">
                    <h4 className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-3">{tt("version_history", "Version History")}</h4>
                      ...
                      {doc.versions.map((v, i) => (
                        <div key={v.id} className="relative flex items-center gap-3 pl-8">
                          ...
                          <div className="flex-1 flex justify-between items-center bg-white dark:bg-slate-900 p-2.5 rounded-lg border text-xs shadow-sm">
                            <div>
                              <span className="font-semibold mr-2">{tt("version_label", "Version")} {v.versionNumber}</span>
