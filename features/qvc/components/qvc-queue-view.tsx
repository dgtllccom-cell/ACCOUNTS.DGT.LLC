...
const FILTERS = [
  { key: "qvc_pending", labelKey: "qvc.pending", labelFallback: "Pending" },
  { key: "needs_review", labelKey: "qvc.needs_review", labelFallback: "Needs Review" },
  { key: "qvc_approved", labelKey: "qvc.approved", labelFallback: "Approved" },
  { key: "error", labelKey: "qvc.error", labelFallback: "Error" },
  { key: "", labelKey: "qvc.all", labelFallback: "All" },
];
  ...
  return (
    <div className="p-4 sm:p-6 space-y-4" dir={isRtl ? "rtl" : "ltr"}>
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-bold text-slate-900 dark:text-slate-100">{tt("qvc.title", "QVC Review Queue")}</h1>
          <p className="text-xs text-slate-500">{tt("qvc.subtitle", "Excel → Import Completed → QVC Pending → Manual Review → QVC Approved. Review each account, complete translations, then approve.")}</p>
        </div>
        <div className="flex items-center gap-2">
          <div className="relative">
            <Search className="absolute left-2 top-2 h-3.5 w-3.5 text-slate-400" />
            <input value={q} onChange={e => setQ(e.target.value)} placeholder={tt("qvc.search", "Search manual # / name / code")} className="w-64 rounded-lg border border-slate-200 py-1.5 pl-7 pr-2 text-xs dark:border-slate-700 dark:bg-slate-900" />
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-1.5">
        {FILTERS.map(f => (
          <button key={f.key} onClick={() => setFilter(f.key)}
            ...
            {tt(f.labelKey, f.labelFallback)}{f.key && counts[f.key] != null ? ` (${counts[f.key]})` : ""}
          </button>
        ))}
      </div>
        ...
        <table className="w-full min-w-[1400px] text-xs">
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            {loading ? (
              ...
              <tr><td colSpan={15} className="px-3 py-12 text-center text-slate-400">{tt("qvc.empty", "No accounts in this queue.")}</td></tr>
            ) : rows.map(r => (
              <tr key={r.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                ...
                <td className="px-3 py-2.5 text-center whitespace-nowrap">
                  <button onClick={() => setStatus(r.id, "qvc_approved")} className="inline-flex items-center gap-1 rounded bg-emerald-600 px-2 py-1 text-[10px] font-bold text-white hover:bg-emerald-500 mr-1" title={tt("qvc.approve", "Approve")}><CheckCircle2 className="h-3 w-3" /> {tt("qvc.approve", "Approve")}</button>
                  <button onClick={() => setStatus(r.id, "error")} className="inline-flex items-center gap-1 rounded bg-rose-600 px-2 py-1 text-[10px] font-bold text-white hover:bg-rose-500" title={tt("qvc.error_flag", "Flag error")}><AlertTriangle className="h-3 w-3" /> {tt("qvc.error_flag", "Error")}</button>
                </td>
              </tr>
            ))}
          </tbody>
