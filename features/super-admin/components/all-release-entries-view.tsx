...
export function AllReleaseEntriesView({ lang: langProp = "en" }: { lang?: string }) {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await apiGet<ActivityResponse>(`/api/erp/super-admin/activity?module=${encodeURIComponent(moduleFilter)}&limit=150`);
      setData(res);
    } catch (e: any) {
  ...
  const entries = useMemo(() => {
    ...
    return list.filter((e) =>
      [e.module, e.country, e.branch, e.entryName, e.party, e.bank, e.reference, e.status]
  ...
  function printSelected() {
    ...
    openJournalReportWindow({
      ...
      reportIdPrefix: "ERP",
      reportIdValue: selected.reference || selected.id || "entry",
      chips: [
      ...
      rows: [
        ...
        { field: tt("sae.linked_bank", "Linked Bank"), value: selected.bank },
        ...
        { field: tt("sae.final", "Balance / Final"), value: selected.final },
    ...
    <div className="space-y-4 text-slate-900 dark:text-slate-100" dir={isRtl ? "rtl" : "ltr"}>
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
        <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3 dark:border-slate-800">
          ...
          <span className="text-[11px] text-slate-500">{tt("sae.click_row", "Click any row to open full detail")} · {entries.length}</span>
        </div>
          ...
          <aside className={`fixed top-0 z-50 h-full w-full max-w-md overflow-auto bg-white p-5 shadow-2xl dark:bg-slate-900 ${isRtl ? "left-0" : "right-0"}`} dir={isRtl ? "rtl" : "ltr"}>
            ...
            <div className="mt-4 grid grid-cols-2 gap-2">
              {([
                ...
                [tt("sae.linked_bank", "Linked Bank"), selected.bank],
                ...
                [tt("sae.final", "Balance / Final"), selected.final],
