...
export function openConsignmentReport(report: ConsignmentReport, lang: string) {
  ...
  const containersHtml = subTable(
    ...
    [
      ...
      t(lang, "cns.reference_value", "Reference Value"),
      t(lang, "cns.container_status", "Container Status"),
      t(lang, "cns.status", "Sales Status"),
    ...
    report.containers.map((ct) => [
      ...
      ct.reference_value != null ? n(ct.reference_value).toLocaleString() : "—",
      t(lang, `cns.cs_${ct.status}`, ct.status),
      t(lang, `cns.ss_${ct.sale_status}`, ct.sale_status),
  ...
  const salesHtml = subTable(
    ...
    [t(lang, "cns.sale_date", "Sale Date"), t(lang, "cns.buyer_name", "Buyer / Customer"), t(lang, "cns.goods_name", "Goods Name"), t(lang, "cns.cartons", "Cartons"), t(lang, "cns.quantity", "Qty"), t(lang, "cns.net_weight", "Weight"), t(lang, "cns.rate", "Rate"), t(lang, "cns.amount", "Amount")],
    report.sales.map((sl) => [
      ...
      sl.net_weight != null ? String(sl.net_weight) : "—",
  ...
  openGenericErpReport({
    ...
    filters: [
      ...
      { label: t(lang, "cns.reference_no", "Reference / Invoice No"), value: c.reference_no || "—" },
      { label: t(lang, "cns.tender_no", "Tender / Contract No"), value: c.tender_no || "—" },
      { label: t(lang, "cns.consignment_date", "Date"), value: String(c.consignment_date).slice(0, 10) },
      {
        label: t(lang, "cns.loading_window", "Loading"),
        value: `${String(c.loading_from_date || "—").slice(0, 10)} → ${String(c.loading_to_date || "—").slice(0, 10)}`,
      },
