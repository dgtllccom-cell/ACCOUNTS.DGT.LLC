...
for (const [lang, map] of Object.entries(dictionaries)) {
  const dictHeader = `const ${lang}: Dict = {`;
  const pos = content.indexOf(dictHeader);
  if (pos !== -1) {
    const afterPos = pos + dictHeader.length;
    let newEntries = '\n';
    for (const [k, v] of Object.entries(map)) {
      newEntries += `  "${k}": "${v}",\n`;
    }
    content = content.slice(0, afterPos) + newEntries + content.slice(afterPos);
  }
}

fs.writeFileSync(uiFilePath, content, 'utf8');
console.log('✓ Successfully injected exact dictionary keys into ui.ts');
