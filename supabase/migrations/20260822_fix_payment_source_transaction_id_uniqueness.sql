-- Fixes a critical bug introduced by 20260812_roznamcha_posting_idempotency_and_category.sql:
-- both post_purchase_order_payment and post_sales_order_payment set
-- roznamcha_entries.source_transaction_id = the ORDER's own id for EVERY payment kind
-- (booking, advance, remaining, credit), not just booking. Combined with the unique index
-- on (source_module, source_transaction_type, source_transaction_id), this means a purchase
-- or sales order could only ever receive ONE payment of a given kind, ever — a second
-- legitimate advance top-up, remaining/second payment, or credit payment against the same
-- order was rejected as a false "duplicate posting", even though the application code's own
-- comment states the guard is intended for the 'booking' kind only.
--
-- The sales side was worse: source_transaction_type was hardcoded to 'sales_transfer_to_payment'
-- for every kind (no per-kind differentiation like the purchase side has), and the idempotency
-- guard fired unconditionally — so the FIRST sales payment of ANY kind against a sales order
-- permanently blocked every subsequent payment of any kind against that same order.
--
-- Fix: keep the existing "only one booking ever" behavior (source_transaction_id = order id,
-- guarded pre-check) for the booking kind specifically, since that is a real, intentional
-- control (a purchase/sales order should only be booked once). For every other kind
-- (advance/remaining/credit), use the newly-created roznamcha entry's own id as
-- source_transaction_id — always unique per posting by construction — so the unique index
-- can never falsely collide between two separate legitimate payments of the same kind.
