...
<body>
  <main class="report-page">
    <header class="header">
      <div class="brand"><div class="logo">${logo}</div><div><div class="company">${escapeHtml(meta.companyName ?? "")}</div>${(meta.companyInfo ?? []).map((line) => `<div class="muted">${escapeHtml(line)}</div>`).join("")}</div></div>
    ...
    <footer class="footer"><span>${escapeHtml(input.footerText ?? "Generated by Digital Dock ERP")}</span><span>Page 1</span></footer>
  </main>
</body></html>`;
}
