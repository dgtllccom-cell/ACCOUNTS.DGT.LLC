...
fs.writeFileSync("scripts/vps_del_user.mjs", script);
execSync(`scp -o StrictHostKeyChecking=no scripts/vps_del_user.mjs ${SERVER}:/var/www/dgt-nextjs/vps_del_user.mjs`);
execSync(`ssh -o StrictHostKeyChecking=no ${SERVER} "cd /var/www/dgt-nextjs && node vps_del_user.mjs && rm -f vps_del_user.mjs"`, { stdio: "inherit" });
