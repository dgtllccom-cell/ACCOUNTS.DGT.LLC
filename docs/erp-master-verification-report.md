...
production-readiness, page-title pass, gate re-run. Production promotion is
handed to the operator as a runbook (§ Production Runbook) — this session does
not have the VPS/production-DB access to run it, and doing it blind would
violate the "backups / rollback safety" requirement.
