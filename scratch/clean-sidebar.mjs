...
// 1. Add imports after "lucide-react";
content = content.replace(
  '} from "lucide-react";',
  '} from "lucide-react";\nimport { useActiveLanguage } from "@/lib/i18n/use-active-language";\nimport { translateHeader } from "@/lib/i18n/table-headers";'
);

// 2. Add useTr helper before internal components
content = content.replace(
  '/* ---------------- internal components ---------------- */',
...
content = content.replace('<span className="flex-1 truncate text-left">{item.label}</span>', '<span className="flex-1 truncate text-left">{tr(item.label)}</span>');
...
content = content.replace('<Icon className="h-3 w-3" />\n        {title}', '<Icon className="h-3 w-3" />\n        {tr(title)}');
content = content.replace('<span className="truncate">{it.label}</span>', '<span className="truncate">{tr(it.label)}</span>');

// 5. In DigitalDockPremiumSidebar
content = content.replace(
...
console.log("Successfully transformed sidebar!");
