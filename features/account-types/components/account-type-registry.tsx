  ...
  return (
    ...
    <Card className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm">
      <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
        <div>
          <CardTitle className="text-xl font-bold text-slate-900 dark:text-slate-100">Account Types</CardTitle>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Manage chart of accounts classification categories and ledger groups</p>
        </div>
        <div className="flex gap-2">
          ...
          <Button onClick={() => setIsModalOpen(true)} size="sm" className="bg-emerald-600 hover:bg-emerald-700 text-white">
            <Plus className="w-4 h-4 mr-1" /> + New Account Type
          </Button>
        </div>
      </CardHeader>
      <CardContent className="space-y-4 pt-4">
        <div className="flex gap-2">
          <Input placeholder="Search account types..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="max-w-md" />
          ...
          <div className="border border-slate-200 dark:border-slate-800 rounded-lg overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="p-8 text-center text-slate-500">No account types found</td>
                  </tr>
                ) : (
                  filtered.map((type, idx) => (
                    <tr key={type.id} className="border-b border-slate-100 dark:border-slate-800/50 hover:bg-slate-50/60 dark:hover:bg-slate-800/40">
                      ...
                      <td className="p-3 text-center">
                        <button onClick={() => handleDelete(type.id)} className="p-1.5 hover:bg-rose-50 dark:hover:bg-rose-950 text-rose-600 dark:text-rose-400 rounded transition-colors" title="Delete Account Type">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-800 w-full max-w-lg overflow-hidden">
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800">
            <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
              <Landmark className="w-5 h-5 text-emerald-600" /> Add New Account Type
            </h3>
          ...
          <form onSubmit={handleSave} className="p-6 space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  placeholder="e.g. ASSET-CURR / EXP-OPR"
                  value={formData.code}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-600 dark:text-slate-300 uppercase tracking-wider mb-1">Ledger Group</label>
                <select
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-600 dark:text-slate-300 uppercase tracking-wider mb-1">Account Type Name *</label>
              <Input
            ...
            <div>
              ...
              <Input
                placeholder="Optional description / accounting guidance..."
            ...
            <div className="flex items-center gap-2 pt-2">
              ...
              />
              <label htmlFor="acct_is_active" className="text-sm font-medium text-slate-700 dark:text-slate-300">Active Account Type</label>
            </div>
    ...
    <UniversalReportModal
      ...
      title="Account Types Registry"
      data={types}
      columns={[
        { key: "code", label: "Code" },
        { key: "name", label: "Type Name" },
        { key: "ledger_group", label: "Ledger Group" },
