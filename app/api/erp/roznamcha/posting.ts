...
// Roznamcha posting engine — extracted from route.ts so the route module only
// exports Next.js route handlers (a non-handler export from route.ts fails the
// generated .next/types route-shape check).
...
export async function localizeRoznamchaEntryLines<T extends { id: string; roznamcha_lines?: Array<{ id: string; description?: string | null }> | null }>(
  entries: T[],
...
export function createOperationalPostingPlan(body: ReturnType<typeof roznamchaPostingSchema.parse>) {
