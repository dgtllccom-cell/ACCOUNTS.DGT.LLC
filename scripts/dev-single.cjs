...
async function main() {
  ...
  // Purge only when KEEP_NEXT_CACHE != "1" AND no valid prerender-manifest exists.
  // Preserving the cache avoids the webpack chunk hash invalidation cascade that
  // breaks all lazy-compiled routes after the first _not-found/_error recompile.
  const prerenderManifest = path.join(nextDir, "prerender-manifest.json");
  const hasValidCache = fs.existsSync(nextDir) && fs.existsSync(prerenderManifest);
  if (process.env.KEEP_NEXT_CACHE !== "1" && !hasValidCache && fs.existsSync(nextDir)) {
    try {
    ...
    console.log("[dev-single] Reusing existing .next cache (prerender-manifest present).");
  }
