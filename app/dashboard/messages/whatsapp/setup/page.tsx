...
const tw = (lang: string, key: string, fallback: string) => t(lang, key as never, fallback);
...
function ConnectAccountModal({ onClose, onSave }: { onClose: () => void; onSave: (data: ConnectFormData) => Promise<void> }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : tw(lang, "wa.err_connect", "Failed to connect account"));
    } finally {
      setIsSaving(false);
    }
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="w-full max-w-md rounded-2xl bg-card shadow-2xl border border-border">
        <div className="border-b border-border px-5 py-4">
          <h2 className="text-sm font-semibold text-foreground">{tw(lang, "wa.connect_title", "Connect WhatsApp Business Account")}</h2>
          <p className="mt-0.5 text-xs text-muted-foreground">
            {tw(lang, "wa.connect_hint", "You'll need your Meta WhatsApp Cloud API credentials")}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3 p-5">
          {/* Scope */}
          <div>
            <label className="mb-1 block text-[11px] font-medium text-foreground">{tw(lang, "wa.account_scope", "Account Scope")}</label>
            <select
              value={form.scope}
            ...
            >
              <option value="city_branch">{tw(lang, "wa.scope_city_branch", "City Branch")}</option>
              <option value="country_branch">{tw(lang, "wa.scope_country_branch", "Country Branch")}</option>
              <option value="country">{tw(lang, "common.country", "Country")}</option>
              <option value="super_admin">{tw(lang, "wa.scope_global", "Global (Super Admin)")}</option>
            </select>
          </div>

          {[
            { field: "displayName", label: tw(lang, "wa.f_display_name", "Display Name"), placeholder: tw(lang, "wa.f_display_name_ph", "e.g. Branch WhatsApp") },
            { field: "phoneNumber", label: tw(lang, "wa.f_phone", "Phone Number (E.164)"), placeholder: "+000000000000" },
            { field: "phoneNumberId", label: tw(lang, "wa.f_phone_id", "Phone Number ID (from Meta)"), placeholder: "1234567890" },
            { field: "wabaId", label: tw(lang, "wa.f_waba_id", "WhatsApp Business Account ID"), placeholder: "1234567890" },
            { field: "accessToken", label: tw(lang, "wa.f_token", "Permanent System User Access Token"), placeholder: "EAA..." }
          ...
          <div className="flex gap-2 pt-1">
            ...
            >
              ...
              {tw(lang, "wa.connect_account", "Connect Account")}
            </button>
          </div>
        </form>
      </div>
    ...
    return (
      <div className="mx-auto max-w-xl py-12 text-center space-y-4">
        <div className="p-4 rounded-2xl bg-rose-50 text-rose-700 dark:bg-rose-950/40 dark:text-rose-400 border border-rose-200 dark:border-rose-900 font-bold text-sm">
          ⛔ {tw(lang, "wa.access_denied", "Access Denied — WhatsApp Business Account Configuration is strictly restricted to Super Admin.")}
        </div>
        <Link href="/dashboard/return-sms-reply" className="inline-block px-4 py-2 bg-slate-900 text-white rounded-xl text-xs font-bold">
          {tw(lang, "wa.return_inbox", "Return to Inbox")}
        </Link>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      >
        ...
        {tw(lang, "wa.back_inbox", "Back to Inbox")}
      </Link>

      <div>
        <h1 className="text-lg font-bold text-foreground">{tw(lang, "wa.setup_title", "WhatsApp Account Setup")}</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          {tw(lang, "wa.setup_desc", "Connect and manage WhatsApp Business numbers for each country and branch. Uses the official Meta WhatsApp Cloud API — numbers are safe from being blocked.")}
        </p>
      </div>
