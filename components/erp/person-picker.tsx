...
function toOption(row: PersonRow, lang: string = "en"): SearchSelectOption {
  ...
  const fatherRaw = row.father_name || ((row as any).contact_person && (row as any).contact_person !== rawName ? (row as any).contact_person : null);
...
export function PersonPicker({
  ...
  async function handleQuickCreatePerson(name: string) {
    ...
    // Duplicate check before creating a new Person Master row — search existing people by the
    // typed name and warn if a close match already exists, instead of silently re-registering
    // the same person under a second row.
    setLoading(true);
    try {
      const qp = new URLSearchParams();
      qp.set("q", trimmed);
      ...
      qp.set("lang", lang);
      const res = await apiGet<{ customers: PersonRow[] }>(`/api/erp/customers?${qp.toString()}`);
      const matches = (res.customers ?? []).filter((p) => nameMatches(personFullName(p), trimmed));
      if (matches.length > 0) {
        setDupCandidates(matches.map((p) => ({
          id: p.id,
        ...
        setDupSearchedName(trimmed);
        setPendingCreateName(trimmed);
        setLoading(false);
        return;
      }
    } catch {
      // If the duplicate-check search itself fails, fall through to create — never block
      // registration on a search-availability issue.
    }
    await createPersonNow(trimmed);
  }
          ...
          <div className="p-5 space-y-4 text-xs text-slate-800 dark:text-slate-200">
            <div className="flex items-center justify-between bg-slate-50 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 p-4 rounded-2xl">
              <div>
                ...
                {viewPerson.person_code ? (
                  <p className="text-[10px] font-mono font-black text-blue-600 dark:text-blue-400 mt-0.5">{viewPerson.person_code}</p>
                ) : null}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 bg-white dark:bg-slate-950">
              <div>
                <span className="text-[10px] font-bold text-slate-400 uppercase block">{t(lang, "erp.person_dup_father", "Father")}</span>
                <span className="font-bold text-slate-800 dark:text-white text-sm">{viewPerson.father_name ? transliterateProperNoun(viewPerson.father_name, lang) : "—"}</span>
              </div>
              <div>
      ...
      {dupCandidates.length > 0 ? (
        <PersonDuplicateWarningModal
          ...
          onCreateAnyway={() => {
            ...
            setDupCandidates([]);
            setPendingCreateName(null);
            if (name) void createPersonNow(name);
          }}
          onCancel={() => {
            setDupCandidates([]);
            setPendingCreateName(null);
