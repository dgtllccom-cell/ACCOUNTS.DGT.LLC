...
HTTP_STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 10 http://127.0.0.1:${PORT}/ || echo "000")
...
if [ "$HTTP_STATUS" != "200" ] && [ "$HTTP_STATUS" != "302" ] && [ "$HTTP_STATUS" != "307" ]; then
    ...
    # Give 5 seconds for port to open
    sleep 5
    HTTP_RETRY=$(curl -s -o /dev/null -w "%{http_code}" --max-time 10 http://127.0.0.1:${PORT}/ || echo "000")
    if [ "$HTTP_RETRY" != "200" ] && [ "$HTTP_RETRY" != "302" ] && [ "$HTTP_RETRY" != "307" ]; then
        echo "[HEALTHCHECK] Critical: Restart failed (HTTP ${HTTP_RETRY}). Rebuilding Next.js production bundle..."
