...
export function ReportFilterBar({
  ...
  return (
    <div
      className={cn(
        "rounded-2xl border border-slate-200 bg-white/80 backdrop-blur-sm p-3 shadow-sm dark:bg-slate-900/80 dark:border-slate-800",
        "print:hidden"
      )}
      dir={isRTL ? "rtl" : "ltr"}
    >
      <div className="flex flex-wrap items-end gap-3">
        ...
        {showCountryFilter && (
          <div className="min-w-[150px] flex-1 space-y-1">
            ...
            {isCountryLocked ? (
              <div className="flex items-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 dark:bg-emerald-950/30 dark:border-emerald-900">
        ...
        {showBranchFilter && (
          <div className="min-w-[150px] flex-1 space-y-1">
            ...
            {isBranchLocked ? (
              <div className="flex items-center gap-2 rounded-xl border border-violet-200 bg-violet-50 px-3 py-2 dark:bg-violet-950/30 dark:border-violet-900">
        ...
        <div className="ml-auto flex items-center gap-2 self-end">
          <button
            type="button"
            onClick={onReset}
            className="flex h-9 items-center gap-1 rounded-xl border border-slate-200 bg-white px-3 text-[11px] font-bold text-slate-500 transition-colors hover:text-red-600 dark:bg-slate-950 dark:border-slate-800"
          >
            <X className="h-3 w-3" />
            {_("report.filter_reset")}
          </button>
          {onApply && (
          <button
            type="button"
            onClick={onApply}
            className="flex h-9 items-center gap-1.5 rounded-xl bg-indigo-600 px-4 text-xs font-black text-white hover:bg-indigo-700 shadow-sm transition-all"
          >
            <RefreshCw className="h-3 w-3" />
            {_("report.filter_apply")}
          </button>
          )}
        </div>
      </div>
