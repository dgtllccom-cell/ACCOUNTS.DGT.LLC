...
export function SalesOrderPaymentJournal({ mode = "advance" }: { mode?: PaymentMode }) {
  ...
  const customerOptions = useMemo(() => {
    ...
    orders.forEach((o) => {
      ...
      const name = form.customerName || form.buyerName || o.purchase_account_name || o.sales_account_name;
  ...
  const filtered = useMemo(() => {
    ...
    return orders.filter((row) => {
      ...
      if (partyFilter) {
        const p = (row.form_data?.form?.customerName || row.form_data?.form?.buyerName || row.purchase_account_name || row.sales_account_name || "").toLowerCase();
            ...
            <thead>
              <tr className="border-b border-slate-200 bg-slate-100/90 dark:border-slate-800 dark:bg-[#080d1a] text-[10px] font-black uppercase tracking-wider text-slate-600 dark:text-slate-300">
                {[
                  ...
                  { label: "TOTAL AMT", tip: "Total Sales Amount (Original Currency)" },
                  { label: "ADV %", tip: "Advance Percentage" },
                  { label: "ADV AMT", tip: "Sales Advance Amount (Original Currency)" },
                  { label: "REM AMT", tip: "Remaining Sales Amount (Original Currency)" },
