...
export class BanksRepository {
  ...
  async create(input: {
    ...
    const insertRow = {
      ...
      branch_name: input.branchName?.trim() || "Main Branch",
      branch_code: input.branchCode?.trim() || "001",
      branch_code_type: input.branchCodeType?.trim() || "SWIFT",
      short_name: input.shortName?.trim() || input.bankName?.trim() || "BANK",
      account_title: input.accountTitle?.trim() || input.bankName?.trim() || "Account",
      account_number: input.accountNumber?.trim() || "0000000000",
