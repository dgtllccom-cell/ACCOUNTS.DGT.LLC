...
// 2. Add keys to each language dictionary object
for (const [lang, dict] of Object.entries(navTranslations)) {
  const dictSearch = `const ${lang}: Dict = {`;
  if (content.includes(dictSearch)) {
    let entriesToAdd = '';
    ...
    if (entriesToAdd) {
      content = content.replace(dictSearch, `${dictSearch}\n${entriesToAdd}`);
    }
  }
}

fs.writeFileSync(uiFilePath, content, 'utf8');
console.log('✓ Successfully patched 5-language navigation keys in lib/i18n/ui.ts');
