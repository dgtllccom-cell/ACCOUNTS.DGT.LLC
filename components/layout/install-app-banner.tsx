...
export function InstallAppBanner() {
  ...
  const tb = (key: string, fallback: string) => t(lang, key as never, fallback);
  ...
  return (
    <>
      {/* ── Top Install Banner ── */}
      <div
        className="bg-gradient-to-r from-blue-900 via-[#06122d] to-indigo-950 text-white px-4 pt-3.5 pb-3 shadow-md border-b border-blue-800/50 flex flex-wrap items-center justify-between gap-3 text-xs overflow-visible min-h-[52px]"
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-blue-200/90 leading-snug hidden sm:block mt-0.5">
              {tb("iab.subtitle", "Click below to automatically download the app installer directly into your computer for 1-click desktop access.")}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 ml-auto">
          ...
          >
            ...
            {tb("iab.download_now", "DOWNLOAD & INSTALL APP NOW")}
          </Button>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 max-w-sm w-full shadow-2xl border border-slate-200 dark:border-slate-800 space-y-4">
            <div className="flex items-center justify-between border-b pb-3 dark:border-slate-800">
              <h3 className="font-black text-sm text-slate-900 dark:text-white flex items-center gap-2">
                ...
                {tb("iab.ios_title", "Install on iPhone / iPad")}
              </h3>
              <button
            ...
            <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
              {tb("iab.ios_intro", "Follow these 2 simple steps to add the Digital Dock ERP App icon to your iPhone Home Screen:")}
            </p>
            ...
            <div className="space-y-3 bg-slate-50 dark:bg-slate-950 p-4 rounded-xl border border-slate-200 dark:border-slate-800 text-xs">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <span className="font-bold text-slate-800 dark:text-slate-200">{tb("iab.ios_s1", "Tap the Share Button")}</span>
                  <p className="text-[11px] text-slate-500 mt-0.5 flex items-center gap-1">
                    {tb("iab.ios_s1b", "Tap the share icon at the bottom of Safari.")}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <span className="font-bold text-slate-800 dark:text-slate-200">{tb("iab.ios_s2", "Tap Add to Home Screen")}</span>
                  <p className="text-[11px] text-slate-500 mt-0.5 flex items-center gap-1">
                    {tb("iab.ios_s2b", "Scroll down and tap the Add to Home Screen option.")}
                  </p>
                </div>
              </div>
            </div>
            ...
            >
              <CheckCircle2 className="h-4 w-4 mr-1.5" /> {tb("iab.got_it", "GOT IT")}
            </Button>
          </div>
        </div>
      )}
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 max-w-md w-full shadow-2xl border border-slate-200 dark:border-slate-800 space-y-4">
            <div className="flex items-center justify-between border-b pb-3 dark:border-slate-800">
              <h3 className="font-black text-sm text-slate-900 dark:text-white flex items-center gap-2">
                ...
                {tb("iab.dl_done_title", "App Installer Downloaded Automatically")}
              </h3>
              <button
            ...
            <div className="bg-emerald-50 dark:bg-emerald-950/40 p-4 rounded-xl border border-emerald-200 dark:border-emerald-800/50 space-y-2">
              <div className="flex items-center gap-2 text-emerald-800 dark:text-emerald-300 font-bold text-xs">
                ...
                <span>{tb("iab.dl_completed", "Automatic Download Completed!")}</span>
              </div>
              <p className="text-xs text-emerald-700 dark:text-emerald-400 leading-relaxed">
                {tb("iab.dl_body", "The 1-click desktop app installer (Install-Digital-Dock-ERP.bat) has been downloaded into your computer Downloads folder.")}
              </p>
            </div>
            ...
            <div className="space-y-2 bg-slate-50 dark:bg-slate-950 p-4 rounded-xl border border-slate-200 dark:border-slate-800 text-xs">
              <span className="font-bold text-slate-800 dark:text-slate-200 block">{tb("iab.setup_title", "How to complete setup on your PC:")}</span>
              <ol className="list-decimal list-inside space-y-1.5 text-slate-600 dark:text-slate-300 text-[11px] leading-relaxed">
                <li>{tb("iab.setup_1", "Go to your Downloads folder on your computer.")}</li>
                <li>{tb("iab.setup_2", "Double-click Install-Digital-Dock-ERP.bat.")}</li>
                <li>{tb("iab.setup_3", "It will automatically place a 1-click icon on your Desktop & Start Menu!")}</li>
              </ol>
            </div>
            ...
            <div className="flex flex-col gap-2 pt-1">
              ...
              >
                <Zap className="h-4 w-4" /> {tb("iab.redownload_bat", "RE-DOWNLOAD 1-CLICK INSTALLER (.BAT)")}
              </Button>
              <Button
                type="button"
              ...
              >
                <Download className="h-3.5 w-3.5" /> {tb("iab.download_url", "DOWNLOAD .URL DESKTOP ICON")}
              </Button>
              <Button
                type="button"
                ...
                className="w-full h-9 border-slate-300 font-bold text-xs gap-1.5 text-slate-700 dark:text-slate-200"
              >
                <ExternalLink className="h-3.5 w-3.5" /> {tb("iab.download_cmd", "DOWNLOAD STANDALONE WINDOW (.CMD)")}
              </Button>
              <Button
                type="button"
                onClick={() => setShowDesktopGuide(false)}
              ...
              >
                <CheckCircle2 className="h-4 w-4 mr-1.5 text-emerald-600" /> {tb("iab.done", "DONE")}
              </Button>
            </div>
          </div>
        </div>
