...
export function SuperAdminDetailedLedgerView() {
  ...
  useEffect(() => {
    ...
    const fetchOptions = async () => {
      ...
      try {
        const res = await listLedgerReportLedgers({ reportScope: "super_admin", limit: 2000, language: lang });
        ...
        <div className="flex flex-wrap items-center gap-3 pt-1">
          ...
          <div className="flex-1 min-w-[280px] max-w-xl">
            <SearchSelect
              ...
              placeholder={searchingLedgers ? tr("Loading all ERP accounts...") : tr("Search by Account Name, No, Ref, Company, Code...")}
            />
          </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          ...
          <div className="space-y-1.5 text-xs">
            <div className="flex justify-between gap-2">
              ...
              <span className="font-bold text-slate-900 dark:text-slate-100 text-end truncate">
                {header?.cityBranchName || header?.countryBranchName || tr("Global Main Branch")}
              </span>
            </div>
            <div className="flex justify-between gap-2">
              <span className="text-slate-400 font-medium">{tr("Country")}:</span>
              <span className="font-semibold text-slate-800 dark:text-slate-200">
                {header?.countryName || tr("Global (All)")}
              </span>
            </div>
            <div className="flex justify-between gap-2">
