...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Batch insert purchase orders into DB
    const insertedOrders = await sql`
      INSERT INTO purchase_orders ${sql(allTestBills.map(b => ({
        purchase_order_no: b.purchase_order_no,
        purchase_contract_no: b.purchase_contract_no,
        country_id: b.country_id,
        country_branch_id: b.country_branch_id,
        city_branch_id: b.city_branch_id,
        status: b.status,
        ledger_posting_status: b.ledger_posting_status,
        order_total: b.order_total,
        currency_code: b.currency_code,
        form_data: sql.json(b.form_data)
      })))}
    ...
    const rozEntriesPayload = transferredOrders.map(ord => {
      ...
      return {
        ...
        country_branch_id: cbId,
        city_branch_id: citybId,
        type: rozType,
        journal_no: `JO-PURCHASE-${ord.purchase_order_no}`,
        voucher_no: `VO-PURCHASE-${ord.purchase_order_no}`,
        entry_date: new Date().toISOString().slice(0, 10),
        reference_no: ord.purchase_order_no,
        narration: `Purchase Bill Transfer: ${ord.purchase_order_no} | Contract: ${ord.purchase_contract_no}`,
        status: 'posted',
        source_module: 'purchase',
        source_transaction_type: 'purchase_booking_transfer',
        source_transaction_id: ord.id
      };
    });

    const insertedRozEntries = await sql`
      INSERT INTO roznamcha_entries ${sql(rozEntriesPayload)}
      RETURNING id, reference_no;
    `;

    const rozLinesPayload: any[] = [];
    let totalDebitSum = 0;
    let totalCreditSum = 0;

    for (const re of insertedRozEntries) {
      const ord = transferredOrders.find(o => o.purchase_order_no === re.reference_no);
      if (!ord) continue;
      const form = ord.form_data?.form || {};
      ...
      // DR Line
      rozLinesPayload.push({
        roznamcha_entry_id: re.id,
        ledger_id: purchaseLedger?.id || purchaseAccount?.id || null,
        debit: amount,
        credit: 0,
        currency: curr,
        payment_entry_type: 'debit',
        description: `DR: Purchase Account (${ord.purchase_order_no}) - Contract: ${ord.purchase_contract_no}`
      });

      // CR Line
      rozLinesPayload.push({
        roznamcha_entry_id: re.id,
        ledger_id: salesLedger?.id || salesAccount?.id || null,
        debit: 0,
        credit: amount,
        currency: curr,
        payment_entry_type: 'credit',
        description: `CR: Sales / Payable Account (${ord.purchase_order_no}) - Contract: ${ord.purchase_contract_no}`
      });
    ...
    if (rozLinesPayload.length > 0) {
      await sql`INSERT INTO roznamcha_lines ${sql(rozLinesPayload)}`;
    }
