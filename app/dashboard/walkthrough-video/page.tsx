...
export default function WalkthroughVideoPage() {
  ...
  const tt = (key: string, fallback: string) => t(lang, ("wtv." + key) as never, fallback);
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 rounded-2xl bg-slate-900 text-white p-6 shadow-xl border border-slate-800">
        <div>
          <div className="flex items-center gap-2 text-blue-400 text-xs font-black uppercase tracking-wider mb-2">
            ...
            <span>{tt("banner_eyebrow", "Digital Dock ERP • System Walkthrough Video")}</span>
          </div>
          <h1 className="text-2xl sm:text-3xl font-black tracking-tight text-balance">
            {tt("title", "System Walkthrough & Operating Guide")}
          </h1>
          <p className="mt-1 text-sm text-slate-300 max-w-3xl">
            {tt("subtitle", "Interactive recording of ERP workflows, navigation, multi-language switching, master setup, Roznamcha, sales, purchases, and accounting reports.")}
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          >
            <Button className="bg-blue-600 hover:bg-blue-500 text-white font-bold gap-2 text-xs shadow-lg">
              ...
              {tt("download_video", "Download Video File (18 MB)")}
            </Button>
          </a>
        </div>
      </div>
      ...
      <Card className="border-slate-200 dark:border-slate-800 shadow-xl overflow-hidden bg-slate-950">
        <CardHeader className="border-b border-slate-800 bg-slate-900/60 px-6 py-4 flex flex-row items-center justify-between">
          <CardTitle className="text-sm font-bold text-white flex items-center gap-2">
            ...
            <span>{tt("live_stream_title", "Live System Walkthrough Stream")}</span>
          </CardTitle>
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-500/10 text-emerald-400 text-xs font-mono font-bold">
            ...
            {tt("production_ready", "100% Production Ready")}
          </span>
        </CardHeader>
        <CardContent className="p-0 flex items-center justify-center min-h-[480px] bg-black">
          <div className="relative w-full overflow-hidden flex flex-col items-center">
            ...
            <img
              ...
              alt={tt("video_alt", "Digital Dock ERP Complete Interactive System Walkthrough Video")}
        ...
        <Card className="border-slate-200 dark:border-slate-800 shadow-md">
          <CardHeader className="pb-3">
            <CardTitle className="text-sm font-bold flex items-center gap-2 text-slate-800 dark:text-slate-100">
              ...
              <span>{tt("card1_title", "Multi-Language & RTL Engine")}</span>
            </CardTitle>
          </CardHeader>
          <CardContent className="text-xs text-slate-600 dark:text-slate-400 space-y-2">
            <p>
              {tt("card1_p1", "Demonstrates 5-language switcher (English, Urdu 🇵🇰, Pashto 🇦🇫, Arabic 🇦🇪, Persian 🇮🇷).")}
            </p>
            <p>
              {tt("card1_p2", "Technical fields (Email, User ID, Passwords, URLs) remain in English while all business modules adapt to locale & RTL.")}
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="border-slate-200 dark:border-slate-800 shadow-md">
          <CardHeader className="pb-3">
            <CardTitle className="text-sm font-bold flex items-center gap-2 text-slate-800 dark:text-slate-100">
              ...
              <span>{tt("card2_title", "Roznamcha & Multi-Currency")}</span>
            </CardTitle>
          </CardHeader>
          <CardContent className="text-xs text-slate-600 dark:text-slate-400 space-y-2">
            <p>
              {tt("card2_p1", "Daily cash entries, bank vouchers, Money Exchange (Buy/Sell rate formulas), and expense bills.")}
            </p>
            <p>
              {tt("card2_p2", "Automatic Debit/Credit balancing and double-entry ledger postings.")}
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="border-slate-200 dark:border-slate-800 shadow-md">
          <CardHeader className="pb-3">
            <CardTitle className="text-sm font-bold flex items-center gap-2 text-slate-800 dark:text-slate-100">
              ...
              <span>{tt("card3_title", "Logistics & Master Setup")}</span>
            </CardTitle>
          </CardHeader>
          <CardContent className="text-xs text-slate-600 dark:text-slate-400 space-y-2">
            <p>
              {tt("card3_p1", "Purchase Booking, Truck Loading, Clearing Agent forms, Commercial Invoices, and BL trade documents.")}
            </p>
            <p>
              {tt("card3_p2", "100% database-driven master forms (Companies, Customers, Banks, Warehouses, Products, Employees).")}
            </p>
          </CardContent>
        </Card>
      </div>
