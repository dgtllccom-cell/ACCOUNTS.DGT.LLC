-- Migration: 20261010_purchase_sales_transfer_currency_and_serials.sql
-- Goal: 
-- 1. Generate 4 distinct scoped serials (Super Admin, Country, Branch, Entry) for each DR and CR line.
-- 2. Post approved final converted amount in final currency exactly once, avoiding double conversion.
-- 3. Harmonize purchase and sales booking transfer functions.

-- 1. Upgrade post_roznamcha_entry with independent 4-level serials per line
...
-- 2. Upgrade post_purchase_order_payment to eliminate double conversion
CREATE OR REPLACE FUNCTION public.post_purchase_order_payment(
...
BEGIN
  ...
  -- If p_amount is already passed in the final local currency (e.g. AED)
  -- OR matches the order_total, do not multiply by exchange rate again!
...
-- 3. Upgrade post_sales_order_payment to eliminate double conversion and post in final currency
