...
console.log("Running Deep Inspection and Complete Cleanup on VPS Production...");
...
fs.writeFileSync("scripts/vps_deep_clean.mjs", script);

try {
  execSync(`scp -o StrictHostKeyChecking=no scripts/vps_deep_clean.mjs ${SERVER}:/var/www/dgt-nextjs/vps_deep_clean.mjs`, { stdio: "inherit" });
  execSync(`ssh -o StrictHostKeyChecking=no ${SERVER} "cd /var/www/dgt-nextjs && node vps_deep_clean.mjs && pm2 restart dgt-nextjs && rm -f vps_deep_clean.mjs"`, { stdio: "inherit" });
} catch (e) {
