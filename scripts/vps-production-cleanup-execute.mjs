...
fs.writeFileSync("scripts/remote_run_clean.mjs", localCleanupScript);

try {
  execSync(`scp -o StrictHostKeyChecking=no scripts/remote_run_clean.mjs ${SERVER}:/var/www/dgt-nextjs/remote_run_clean.mjs`, { stdio: "inherit" });
  execSync(`ssh -o StrictHostKeyChecking=no ${SERVER} "cd /var/www/dgt-nextjs && node remote_run_clean.mjs && rm -f remote_run_clean.mjs"`, { stdio: "inherit" });
} catch (e) {
