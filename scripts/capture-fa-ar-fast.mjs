...
async function main() {
  ...
  // 1. Log in using exact working selector sequence
  await page.goto("http://localhost:3000/auth/login", { waitUntil: "networkidle" });
  await page.waitForSelector('input[name="identifier"], input[type="text"]', { state: "visible" });
  ...
  for (const lang of LANGUAGES) {
    ...
    // Roznamcha All
    await page.goto("http://localhost:3000/dashboard/roznamcha/reports/all", { waitUntil: "networkidle" });
    ...
    // Business Roznamcha
    await page.goto("http://localhost:3000/dashboard/roznamcha/reports/business", { waitUntil: "networkidle" });
    await page.waitForTimeout(1500);
    const bizShotPath = `C:/Users/dgtll/.gemini/antigravity-ide/brain/7c714624-7346-45ba-a2cc-e298bf3f1ed4/screenshots/live_roznamcha_business_${lang.code}.png`;
    await page.screenshot({ path: bizShotPath, fullPage: false });
    console.log(`Saved screenshot: ${bizShotPath}`);
  }

  await browser.close();
  console.log("Finished AR and FA capture!");
}
