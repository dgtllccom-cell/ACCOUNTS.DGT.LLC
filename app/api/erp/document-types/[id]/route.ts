...
export async function PUT(
  ...
  try {
    const session = await requireErpSession();
    authorizeApiScope(session, { resource: "document_types", action: "update" });
    ...
    return apiOk({ documentType: updated });
  } catch (error) {
    return handleApiError(error);
  }
}
...
export async function DELETE(
  ...
  try {
    const session = await requireErpSession();
    authorizeApiScope(session, { resource: "document_types", action: "delete" });
