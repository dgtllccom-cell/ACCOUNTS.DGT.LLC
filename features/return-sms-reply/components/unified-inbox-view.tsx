...
export function UnifiedInboxView({ lang }: Props) {
  ...
  return (
    <div className="space-y-4" dir={isRTL ? "rtl" : "ltr"}>
      ...
      {/* Tabs bar */}
      <div className="flex items-center gap-2 overflow-x-auto px-0.5 pb-2 border-b dark:border-slate-800">
        {INBOX_TABS.map((tab) => (
          <button
            key={tab.key}
            onClick={() => setActiveTab(tab.key)}
            className={cn(
              "shrink-0 rounded-xl border px-3.5 py-2 text-xs font-bold whitespace-nowrap transition-all flex items-center gap-1.5",
              activeTab === tab.key
