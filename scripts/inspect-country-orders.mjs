...
async function main() {
  console.log("=== CHECKING ORDERS IN AFGHANISTAN, PAKISTAN, UAE ===");
  
  const pos = await sql`
    SELECT po.id, po.purchase_order_no, po.country_id, c.name as country_name,
           po.country_branch_id, cb.name as branch_name,
  ...
  `;
  console.log("Purchase Orders by Country:");
  console.table(pos);

  const sos = await sql`
    SELECT so.id, so.sales_order_no, so.country_id, c.name as country_name,
           so.country_branch_id, cb.name as branch_name,
  ...
  `;
  console.log("Sales Orders by Country:");
  console.table(sos);
