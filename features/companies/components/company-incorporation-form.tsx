...
function PreviewField({
  ...
  const hasValue = Boolean(value && typeof value === "string" && value.trim() && value !== "-" && value !== "—" && !value.includes("€") && !value.includes("â"));
  return (
    <div className={className}>
      <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">{label}</p>
      <p
        className={`mt-1 break-words text-sm leading-snug ${mono ? "font-mono" : "font-sans"} ${
          hasValue ? "font-bold text-slate-800" : "font-bold text-slate-300"
        }`}
      >
        {hasValue ? value : "-"}
      </p>
