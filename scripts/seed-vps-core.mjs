...
fs.writeFileSync("scripts/vps_seed_core.mjs", script);

try {
  execSync(`scp -o StrictHostKeyChecking=no scripts/vps_seed_core.mjs ${SERVER}:/var/www/dgt-nextjs/vps_seed_core.mjs`, { stdio: "inherit" });
  execSync(`ssh -o StrictHostKeyChecking=no ${SERVER} "cd /var/www/dgt-nextjs && node vps_seed_core.mjs && rm -f vps_seed_core.mjs"`, { stdio: "inherit" });
} catch (e) {
