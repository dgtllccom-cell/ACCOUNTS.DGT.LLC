  ...
  return (
    <div className="mx-auto w-full bg-[#f8fbff] dark:bg-slate-950/40 text-slate-950 dark:text-slate-50 min-h-screen">
      ...
      <div className="space-y-4 px-4 pb-4">
        {/* PROMINENT ROLE-BASED FINANCIAL SUMMARY SECTION */}
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950">
          {loadingOverview && !summaryOverview ? (
            <div className="p-6 text-center text-xs font-semibold text-slate-400 animate-pulse">
              Loading Financial Position & Country Summaries...
            </div>
          ) : summaryOverview?.isSuperAdmin || session?.scopes?.isSuperAdmin ? (
            /* SUPER ADMIN VIEW: Every Country Separately */
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {(summaryOverview?.countries || countries.map(c => ({
                  countryId: c.id,
                  countryName: c.name,
                  iso2: c.iso2,
                  ...
                  balanceType: "-",
                  entryCount: 0,
                  rates: { buyingRate: null, sellingRate: null, creditRate: null, debitRate: null },
                  branches: []
                }))).map((c: any) => {
                  const isSelected = expandedCountryId === c.countryId || countryId === c.countryId;
                  const flag = getCountryFlag(c.countryName);

                  return (
                    <Card
                      key={c.countryId}
                      onClick={() => {
                        setExpandedCountryId(isSelected ? null : c.countryId);
                        setCountryId(c.countryId);
                      }}
                      className={cn(
                        "cursor-pointer transition-all duration-200 overflow-hidden border-2 shadow-xs hover:shadow-md",
                        isSelected
                          ? "border-blue-500 bg-blue-50/30 dark:bg-blue-950/20 ring-2 ring-blue-500/20"
                          : "border-slate-200 hover:border-blue-300 bg-white dark:border-slate-800 dark:bg-slate-950"
                      )}
                    >
                      {/* Card Header */}
                      <div className="border-b border-slate-100 bg-slate-50/80 px-4 py-2.5 dark:border-slate-800 dark:bg-slate-900/50 flex justify-between items-center">
                        <div className="flex items-center gap-2">
                          <span className="text-xl">{flag}</span>
                          <div>
                            <h3 className="text-xs font-black text-slate-800 dark:text-slate-100">{c.countryName}</h3>
                            <span className="text-[10px] font-bold text-slate-400">{c.branches?.length || 0} Branches</span>
                          </div>
                        </div>
                        <span className="font-mono text-xs font-black bg-blue-100 dark:bg-blue-950 text-blue-800 dark:text-blue-300 px-2 py-0.5 rounded">
                          {c.currencyCode}
                        </span>
                      </div>
                      ...
                      <CardContent className="p-3 space-y-2.5">
                        <div className="grid grid-cols-2 gap-2.5 text-xs">
                          <div className="bg-emerald-50/80 dark:bg-emerald-950/30 p-2.5 rounded-lg border border-emerald-100 dark:border-emerald-900/50">
                            <span className="text-[10px] font-black uppercase text-emerald-700 dark:text-emerald-400 block">Total Credit (Paid)</span>
                            <span className="text-xs font-black text-emerald-800 dark:text-emerald-300 mt-0.5 block truncate">
                              {fmtAmount(c.totalCredit)} <span className="text-[9px] font-normal">{c.currencyCode}</span>
                            </span>
                          </div>

                          <div className="bg-rose-50/80 dark:bg-rose-950/30 p-2.5 rounded-lg border border-rose-100 dark:border-rose-900/50">
                            <span className="text-[10px] font-black uppercase text-rose-700 dark:text-rose-400 block">Total Debit (Recv)</span>
                            <span className="text-xs font-black text-rose-800 dark:text-rose-300 mt-0.5 block truncate">
                              {fmtAmount(c.totalDebit)} <span className="text-[9px] font-normal">{c.currencyCode}</span>
                            </span>
                          </div>
                        </div>

                        <div className="flex justify-between items-center bg-slate-50 dark:bg-slate-900 p-2.5 rounded-lg border border-slate-200/80 dark:border-slate-800 text-xs">
                          <div>
                            <span className="text-[10px] font-black uppercase text-slate-500 block">Current Balance</span>
                            <span className="text-sm font-black text-slate-800 dark:text-slate-100 font-mono">
                              {c.balanceRaw < 0 ? `-${fmtAmount(c.balance)}` : fmtAmount(c.balance)} {c.currencyCode}
                            </span>
                          </div>
                          <div className="text-right">
                            <span className={cn(
                              "px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-wider",
                              c.balanceType === "Cr" ? "bg-emerald-100 text-emerald-800" : c.balanceType === "Dr" ? "bg-rose-100 text-rose-800" : "bg-slate-200 text-slate-700"
                            )}>
                              {c.balanceType}
                            </span>
                            <span className="text-[10px] font-bold text-slate-500 block mt-0.5">{c.entryCount} Entries</span>
                          </div>
                        </div>

                        {/* Daily Exchange Rates Section */}
                        <div className="pt-2 border-t border-slate-100 dark:border-slate-800 space-y-1">
                          <span className="text-[9px] font-black uppercase tracking-wider text-slate-400 block">Today's Exchange Rates</span>
                          <div className="grid grid-cols-4 gap-1 text-[10px] text-center font-mono">
                            <div className="bg-emerald-50 dark:bg-emerald-950/40 p-1 rounded border border-emerald-100 dark:border-emerald-900">
                              <span className="text-[8px] font-black text-emerald-700 dark:text-emerald-400 block">Buying</span>
                              <span className="font-bold text-slate-800 dark:text-slate-200">{c.rates?.buyingRate ? c.rates.buyingRate.toFixed(2) : "—"}</span>
                            </div>
                            <div className="bg-rose-50 dark:bg-rose-950/40 p-1 rounded border border-rose-100 dark:border-rose-900">
                              <span className="text-[8px] font-black text-rose-700 dark:text-rose-400 block">Selling</span>
                              <span className="font-bold text-slate-800 dark:text-slate-200">{c.rates?.sellingRate ? c.rates.sellingRate.toFixed(2) : "—"}</span>
                            </div>
                            <div className="bg-indigo-50 dark:bg-indigo-950/40 p-1 rounded border border-indigo-100 dark:border-indigo-900">
                              <span className="text-[8px] font-black text-indigo-700 dark:text-indigo-400 block">Credit</span>
                              <span className="font-bold text-slate-800 dark:text-slate-200">{c.rates?.creditRate ? c.rates.creditRate.toFixed(2) : "—"}</span>
                            </div>
                            <div className="bg-amber-50 dark:bg-amber-950/40 p-1 rounded border border-amber-100 dark:border-amber-900">
                              <span className="text-[8px] font-black text-amber-700 dark:text-amber-400 block">Debit</span>
                              <span className="font-bold text-slate-800 dark:text-slate-200">{c.rates?.debitRate ? c.rates.debitRate.toFixed(2) : "—"}</span>
                            </div>
                          </div>
                        </div>

                        {/* Collapsible/Expanded Branch Details */}
                        {isSelected && c.branches && c.branches.length > 0 && (
                          <div className="mt-3 pt-3 border-t border-blue-200 dark:border-blue-900 space-y-1.5" onClick={(e) => e.stopPropagation()}>
                            <span className="text-[10px] font-black uppercase text-blue-700 dark:text-blue-300 block">
                              Branch-wise Breakdown ({c.branches.length})
                            </span>
                            <div className="space-y-1 max-h-[160px] overflow-y-auto pr-1">
                              {c.branches.map((b: any) => (
                                <div
                                  key={b.branchId}
                                  onClick={() => {
                                    setCountryBranchId(b.branchId);
                                  }}
                                  className={cn(
                                    "flex justify-between items-center p-2 rounded text-[11px] border transition-colors cursor-pointer",
                                    countryBranchId === b.branchId ? "bg-blue-100 border-blue-300 dark:bg-blue-900/40" : "bg-white border-slate-100 hover:bg-slate-50 dark:bg-slate-900"
                                  )}
                                >
                                  <div>
                                    <span className="font-bold text-slate-800 dark:text-slate-200 block">{b.branchName}</span>
                                    <span className="text-[9px] text-slate-400 font-mono">{b.branchCode} • {b.entryCount} entries</span>
                                  </div>
                                  <div className="text-right">
                                    <span className="font-bold font-mono text-slate-800 dark:text-slate-200 block">
                                      {b.balanceRaw < 0 ? `-${fmtAmount(b.balance)}` : fmtAmount(b.balance)} {b.localCurrency}
                                    </span>
                                    <span className="text-[9px] text-slate-500 font-semibold">
                                      CR: {fmtAmount(b.totalCredit)} | DR: {fmtAmount(b.totalDebit)}
                                    </span>
                                  </div>
                                </div>
                              ))}
            ...
            /* COUNTRY / BRANCH USER VIEW: Prominent single summary card */
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <h2 className="text-sm font-black uppercase tracking-wider text-slate-800 dark:text-slate-100 flex items-center gap-2">
                  📊 Financial Summary & Cash Position
                  <span className="bg-emerald-100 text-emerald-800 text-[10px] font-bold px-2 py-0.5 rounded-full dark:bg-emerald-950 dark:text-emerald-300">
                    {session?.scopes?.cityBranchIds?.length ? "Branch In-Charge View" : "Country View"}
                  </span>
                </h2>
                <span className="text-[11px] font-semibold text-slate-500">
                  {selectedCityBranch?.name || selectedMainBranch?.name || selectedCountry?.name || "My Branch"} • {entryDate}
                </span>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <Card className="p-4 border-l-4 border-l-emerald-500 bg-white dark:bg-slate-950 shadow-xs">
                  <span className="text-[10px] font-black uppercase text-emerald-700 dark:text-emerald-400 block">Total Credit (Paid)</span>
                  <span className="text-lg font-black text-emerald-800 dark:text-emerald-300 mt-1 block font-mono">
                    {fmtAmount(cashSummary?.totalCredit ?? recentEntriesSummary.totalCredit)} <span className="text-xs font-normal">{branchCurrency}</span>
                  </span>
                </Card>

                <Card className="p-4 border-l-4 border-l-rose-500 bg-white dark:bg-slate-950 shadow-xs">
                  <span className="text-[10px] font-black uppercase text-rose-700 dark:text-rose-400 block">Total Debit (Received)</span>
                  <span className="text-lg font-black text-rose-800 dark:text-rose-300 mt-1 block font-mono">
                    {fmtAmount(cashSummary?.totalDebit ?? recentEntriesSummary.totalDebit)} <span className="text-xs font-normal">{branchCurrency}</span>
                  </span>
                </Card>

                <Card className="p-4 border-l-4 border-l-blue-500 bg-white dark:bg-slate-950 shadow-xs">
                  <span className="text-[10px] font-black uppercase text-blue-700 dark:text-blue-400 block">Current Balance</span>
                  <div className="flex items-baseline justify-between mt-1">
                    <span className="text-lg font-black text-blue-800 dark:text-blue-300 font-mono">
                      {fmtAmount(cashSummary?.balance ?? recentEntriesSummary.balance)} <span className="text-xs font-normal">{branchCurrency}</span>
                    </span>
                    <span className="text-xs font-black text-blue-600 uppercase">
                      {cashSummary?.balanceType ?? recentEntriesSummary.balanceType}
                    </span>
                  </div>
                </Card>

                <Card className="p-4 border-l-4 border-l-indigo-500 bg-white dark:bg-slate-950 shadow-xs">
                  <span className="text-[10px] font-black uppercase text-indigo-700 dark:text-indigo-400 block">Total Entries</span>
                  <span className="text-lg font-black text-indigo-800 dark:text-indigo-300 mt-1 block font-mono">
                    {cashSummary?.entryCount ?? recentEntriesSummary.count} <span className="text-xs font-normal">Records</span>
                  </span>
                </Card>
              </div>
            </div>
