...
export function CompanyRegistrationTypeRegistry() {
  ...
  async function loadTypes() {
    setLoading(true);
    try {
      const res = await apiGet<{ types: CompanyRegistrationTypeRecord[]; summary: typeof summary }>(
  ...
  return (
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          <CardTitle>Company Registration Types</CardTitle>
          <p className="text-sm text-slate-500 mt-1">Manage company registration classification types</p>
        </div>
              ...
              <tbody>
                {filtered.map((type, idx) => (
                  <tr key={type.id} className="border-b hover:bg-slate-50">
                    ...
                    <td className="p-3 text-slate-600">{type.country?.name || '-'}</td>
    ...
    <UniversalReportModal
      ...
      title="Company Registration Type Report"
      subtitle="Master List of Company Registration / Incorporation Types"
      exportFileName="company_registration_types_report"
      filters={[
      ...
      columns={[
        { key: "code", label: "Code" },
        { key: "name", label: "Registration Type Name" },
