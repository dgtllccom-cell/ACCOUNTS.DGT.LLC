...
async function fastSyncTranslationsV2() {
  console.log("=================================================================================");
  console.log("     FAST TRANSLATION SYNC & RECONCILIATION V2 (LOCAL → VPS)                     ");
  console.log("=================================================================================\n");
  ...
  // Insert in parallel batches of 100 with single parameterized queries
  let count = 0;
  const batchSize = 100;
  for (let i = 0; i < cleanTrans.length; i += batchSize) {
      ...
      try {
        await vpsSql`
          INSERT INTO public.record_translations (
            ...
            ${r.id}, ${r.record_table}, ${r.record_id}, ${r.field_name}, ${r.original_text}, ${r.original_language_code},
            ${r.english_text}, ${r.urdu_text}, ${r.arabic_text}, ${r.persian_text}, ${r.pashto_text}
          )
    ...
    if ((i + batchSize) % 1000 < batchSize) {
      console.log(`Progress: ${Math.min(i + batchSize, cleanTrans.length)} / ${cleanTrans.length} synced...`);
    }
  }

  console.log(`✓ Processed ${count} / ${cleanTrans.length} translations.`);
...
fastSyncTranslationsV2().catch(err => {
  console.error("Fast sync v2 error:", err);
  process.exit(1);
});
