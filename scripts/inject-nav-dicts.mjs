...
for (const [lang, dict] of Object.entries(navTranslations)) {
  const dictMarker = `const ${lang}: Dict = {`;
  if (content.includes(dictMarker)) {
    let lines = '';
    for (const [k, v] of Object.entries(dict)) {
      lines += `  "${k}": "${v}",\n`;
    }
    content = content.replace(dictMarker, `${dictMarker}\n${lines}`);
  }
}

fs.writeFileSync(uiFilePath, content, 'utf8');
console.log('✓ Successfully injected localized nav keys into all language dicts!');
